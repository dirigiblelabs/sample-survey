	...
	this.init = function(containerEl, viewNames, id, reconstruct){
		...
		if(id){
			if(!reconstruct){
				...
				var savedState = localStorage.getItem('DIRIGIBLE.application.survey-admin.'+ id);
				if(savedState !== null) {
		...
		let focusView = localStorage.getItem("DIRIGIBLE.application.survey-admin.focusView");
		...
		if(id){
			this.layout.on('stateChanged', function(){
				...
				localStorage.setItem('DIRIGIBLE.application.survey-admin.'+ id, state );
			}.bind(this));			
		}
