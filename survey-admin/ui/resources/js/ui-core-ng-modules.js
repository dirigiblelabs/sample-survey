...
.service('Perspectives', ['$resource', function($resource){
	return $resource(relativePath + 'services/v4/js/survey-admin/api/launchpad/perspectives.js');
}])
.service('Tiles', ['$resource', function($resource){
	return $resource(relativePath + 'services/v4/js/survey-admin/api/launchpad/tiles.js');
}])
.service('Menu', ['$resource', function($resource){
	return $resource(relativePath + 'services/v4/js/survey-admin/api/launchpad/menu.js');
}])
...
.factory('viewRegistry', ['ViewRegistrySvc', '$resource', 'ViewFactories', function(ViewRegistrySvc, $resource, ViewFactories){
	...
	var get = function(viewsExtensionPoint){
		return $resource(relativePath + 'services/v4/js/survey-admin/api/launchpad/views.js')
			.query({
...
.directive('menu', ['$resource', 'Theme', 'User', 'Layouts', 'Menu', 'messageHub', function($resource, Theme, User, Layouts, Menu, messageHub){
	return {
		...
		templateUrl: relativePath + 'services/v4/web/survey-admin/ui/resources/templates/menu.html'
	}
}])
.directive('sidebar', ['Perspectives', function(Perspectives){
	return {
		...
		link: function(scope, el, attrs){
			...
			scope.activePerspective = localStorage.getItem('DIRIGIBLE.application.survey-admin.activePerspective');

			scope.setActivePerspective = function(activePerspective) {
				localStorage.setItem('DIRIGIBLE.application.survey-admin.activePerspective', activePerspective);
				scope.activePerspective = activePerspective;
			};
		},
		templateUrl: relativePath + 'services/v4/web/survey-admin/ui/resources/templates/sidebar.html'
	}
}])
	...
	return {
		...
		link: function(scope, el, attrs){
			...
			scope.setActiveView = function(activeView) {
				localStorage.setItem('DIRIGIBLE.application.survey-admin.activePerspective', activeView.group);
				localStorage.setItem('DIRIGIBLE.application.survey-admin.focusView', activeView.name);
			};
		},
		templateUrl: relativePath + 'services/v4/web/survey-admin/ui/resources/templates/tiles.html'
	}
}])
