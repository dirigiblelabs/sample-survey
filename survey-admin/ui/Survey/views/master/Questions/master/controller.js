...
.factory('$messageHub', [function(){
	...
	var message = function(evtName, data){
		messageHub.post({data: data}, 'survey-admin.Survey.Questions.' + evtName);
	};
	...
	return {
		...
		onEntityRefresh: function(callback) {
			on('survey-admin.Survey.Questions.refresh', callback);
		},
		onQuestionTypesModified: function(callback) {
			on('survey-admin.Survey.QuestionTypes.modified', callback);
		},
		onCampaignsModified: function(callback) {
			on('survey-admin.Survey.Campaigns.modified', callback);
		},
...
.controller('PageController', function ($scope, $http, $messageHub) {
	...
	var api = '../../../../../../../../../../services/v4/js/survey-admin/api/Survey/Questions.js';
	var questiontypeOptionsApi = '../../../../../../../../../../services/v4/js/survey-admin/api/Configurations/QuestionTypes.js';
	var campaignOptionsApi = '../../../../../../../../../../services/v4/js/survey-admin/api/Survey/Campaigns.js';
	...
	function campaignOptionsLoad() {
		$http.get(campaignOptionsApi)
		.success(function(data) {
			$scope.campaignOptions = data;
		});
	}
	campaignOptionsLoad();
	...
	$scope.questiontypeOptionValue = function(optionKey) {
		for (var i = 0 ; i < $scope.questiontypeOptions.length; i ++) {
			if ($scope.questiontypeOptions[i].Id === optionKey) {
				return $scope.questiontypeOptions[i].Type;
			}
		}
		return null;
	};

	$scope.campaignOptionValue = function(optionKey) {
		for (var i = 0 ; i < $scope.campaignOptions.length; i ++) {
			if ($scope.campaignOptions[i].Id === optionKey) {
				return $scope.campaignOptions[i].Name;
			}
		}
		return null;
	};
	...
	$messageHub.onCampaignsModified(campaignOptionsLoad);
