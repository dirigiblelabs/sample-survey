...
.factory('$messageHub', [function(){
	...
	var message = function(evtName, data){
		messageHub.post({data: data}, 'survey-admin.Survey.QuestionOptions.' + evtName);
	};
	...
	return {
		...
		onEntityRefresh: function(callback) {
			on('survey-admin.Survey.QuestionOptions.refresh', callback);
		},
		onQuestionsSelected: function(callback) {
			on('survey-admin.Survey.Questions.selected', callback);
		},
...
.controller('PageController', function ($scope, $http, $messageHub) {
	...
	var api = '../../../../../../../../../../services/v4/js/survey-admin/api/Survey/QuestionOptions.js';
	...
	$scope.loadPage = function(pageNumber) {
		...
		.success(function(data) {
			...
			$http.get(api + '?QuestionId=' + $scope.masterEntityId + '&$offset=' + ((pageNumber - 1) * $scope.dataLimit) + '&$limit=' + $scope.dataLimit)
			.success(function(data) {
				$scope.data = data;
			});
		});
	...
	$scope.create = function() {
		if ($scope.entityForm.$valid) {
			$scope.entity.QuestionId = $scope.masterEntityId;
